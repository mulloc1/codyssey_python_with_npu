...
def _zeros(iSize: int) -> list[list[int]]:
    return [[0 for _ in range(iSize)] for _ in range(iSize)]
...
class TestValidateMatrix(unittest.TestCase):
    ...
    def test_accepts_square_sizes_3_5_13_25(self) -> None:
        for iSize in (3, 5, 13, 25):
            with self.subTest(n=iSize):
                lMatrix = _zeros(iSize)
                self.assertEqual(validate_matrix(lMatrix), iSize)
...
class TestValidateMatrixWithExpectedSize(unittest.TestCase):
    ...
    def test_accepts_when_expected_size_matches(self) -> None:
        for iSize in (3, 5, 13, 25):
            with self.subTest(n=iSize):
                self.assertEqual(validate_matrix(_zeros(iSize), iExpectedSize=iSize), iSize)
    ...
    def test_rejects_when_expected_size_mismatches(self) -> None:
        with self.assertRaises(ValueError) as oCtx:
            validate_matrix(_zeros(3), iExpectedSize=5)
        self.assertIn("expected 5x5", str(oCtx.exception))

    # expected_size는 1 이상이어야 한다.
    def test_rejects_expected_size_less_than_one(self) -> None:
        with self.assertRaises(ValueError):
            validate_matrix(_zeros(1), iExpectedSize=0)

    # expected_size 타입이 int가 아니면 예외가 나야 한다.
    def test_rejects_non_int_expected_size(self) -> None:
        with self.assertRaises(TypeError):
            validate_matrix(_zeros(3), iExpectedSize="3")  # type: ignore[arg-type]
